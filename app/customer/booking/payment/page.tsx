...
export default function PaymentPage() {
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      // 🔥 PERBAIKAN: Gunakan field yang diminta backend
      const requestBody = {
        date: isoDate,
        time: bookingData.time,
        guestCount: Number(bookingData.guests),
        tableId: 1, // table_id = 1 (sudah ada di database)
      };
      ...
      if (!response.ok) {
        const errorMessage = typeof responseData.message === "string" 
          ...
          : JSON.stringify(responseData.message || "Failed to create booking");
        throw new Error(errorMessage);
      }

      const bookingId = responseData.data?.id || responseData.id;
      console.log("Booking ID:", bookingId);

      // Upload payment proof (simpan ke localStorage dulu karena backend mungkin belum ada endpoint)
      if (bookingId && paymentProof) {
        // Simpan bukti pembayaran ke localStorage untuk reference admin
        const paymentData = {
          bookingId: bookingId,
        ...
        localStorage.setItem(`payment_${bookingId}`, JSON.stringify(paymentData));
        console.log("Payment proof saved to localStorage");
      }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-xl border border-[#96683E]/20 shadow-sm overflow-hidden">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between items-center p-3 bg-white rounded-lg border border-[#96683E]/10">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={handleCopyAccount} className="text-[#96683E] hover:text-[#4A3525] transition">
          ...
          <div className="p-6 border-b border-[#96683E]/10">
            ...
            <p className="text-xs text-[#96683E] mb-4">Upload your transfer receipt or payment screenshot (Max 5MB, JPG/PNG)</p>

            <input type="file" ref={fileInputRef} onChange={handleFileSelect} accept="image/*" className="hidden" />

            {!paymentPreview ? (
              <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-[#96683E]/30 rounded-xl p-8 text-center cursor-pointer hover:border-[#96683E] hover:bg-[#EFE9D3]/10 transition bg-[#FDFBF7]">
              ...
              <div className="relative">
                <img src={paymentPreview} alt="Payment proof" className="w-full max-h-64 object-contain rounded-xl border border-[#96683E]/20 bg-[#FDFBF7] p-2" />
                <button onClick={removeFile} className="absolute top-2 right-2 bg-red-500 hover:bg-red-600 text-white rounded-full p-1.5 transition shadow-md">
                  <X size={16} />
                </button>
                <p className="text-xs text-green-600 mt-2 text-center font-medium">✓ Payment proof uploaded — {paymentProof?.name}</p>
              </div>
            )}
          </div>
          ...
          <div className="p-6 border-b border-[#96683E]/10 space-y-4">
            <div className="border border-[#96683E]/15 rounded-xl overflow-hidden">
              <button onClick={() => setIsOccasionOpen(!isOccasionOpen)} className="w-full flex justify-between items-center p-4 text-sm font-semibold text-[#4A3525] hover:bg-[#EFE9D3]/30 transition">
              ...
              {isOccasionOpen && (
                <div className="p-4 pt-0 border-t border-[#96683E]/10">
                  <div className="flex flex-wrap gap-2 mt-3">
                    {occasions.map((occ) => (
                      <button key={occ} onClick={() => setSelectedOccasion(selectedOccasion === occ ? "" : occ)} className={`px-4 py-2 rounded-full text-xs font-semibold transition ${selectedOccasion === occ ? "bg-[#96683E] text-white" : "bg-[#EFE9D3] hover:bg-[#96683E]/20 text-[#4A3525]"}`}>
                        {occ}
                      </button>
                    ))}
                  </div>
                  {selectedOccasion === "Other" && (
                    <input type="text" value={otherOccasion} onChange={(e) => setOtherOccasion(e.target.value)} placeholder="Please specify your occasion..." className="mt-3 w-full p-2 border border-[#96683E]/20 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-[#96683E]" />
                  )}
                </div>
              )}
            </div>
            ...
            <div className="border border-[#96683E]/15 rounded-xl overflow-hidden">
              <button onClick={() => setIsDietaryOpen(!isDietaryOpen)} className="w-full flex justify-between items-center p-4 text-sm font-semibold text-[#4A3525] hover:bg-[#EFE9D3]/30 transition">
                <span>🥗 Dietary Restrictions?</span>
                <ChevronDown className={`transition-transform ${isDietaryOpen ? "rotate-180" : ""}`} size={18} />
              </button>
              {isDietaryOpen && (
                <div className="p-4 pt-0 border-t border-[#96683E]/10">
                  <textarea value={dietaryNote} onChange={(e) => setDietaryNote(e.target.value)} placeholder="e.g., Allergic to nuts, Vegetarian, Gluten-free..." rows={3} className="w-full p-3 border border-[#96683E]/20 rounded-xl text-sm resize-none focus:outline-none focus:ring-1 focus:ring-[#96683E]" />
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-6">
            ...
            <div className="space-y-3 mb-6">
              {[
                ...
                <label key={method.id} className={`flex items-start gap-4 p-4 border rounded-xl cursor-pointer transition ${paymentMethod === method.id ? "border-[#96683E] bg-[#EFE9D3]/20" : "border-[#96683E]/15 hover:bg-[#EFE9D3]/10"}`}>
                  <input type="radio" name="pay" checked={paymentMethod === method.id} onChange={() => setPaymentMethod(method.id)} className="mt-0.5" />
                  <div>
            ...
            {!paymentProof && (
              <p className="text-xs text-red-500 mb-3 text-center">⚠️ Please upload your payment proof before confirming</p>
            )}

            <button onClick={handleConfirmBooking} disabled={isLoading || !paymentProof} className="w-full bg-[#96683E] hover:bg-[#5C321A] text-white py-4 rounded-xl font-bold transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
              {isLoading ? <><Loader2 className="animate-spin" /> Processing...</> : "Confirm & Pay Now"}
            </button>
          </div>
        </div>
      </div>
